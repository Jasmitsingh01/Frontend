...
import { AiOutlineStar ,AiFillCaretLeft,AiFillCaretRight} from "react-icons/ai";
    ...
    const Data = async () => {
      ...
      if (Data.data.operation === "true") {
        ...
        const response = await axios.get(
          `https://powerful-squirrel.railway.internal/user/Catogaries/store?page=${CurrentPage}&limit=4`
        );
...
else if(value.Product_Name.toLowerCase().includes(`${SearchN.toLowerCase()}`)){
      return value;
     }
     else{
     return null
 ...
 return null
  ...
  const First = btoa(
    new Uint8Array(value.Product_Main_Image.data).reduce((data, byte) => data + String.fromCharCode(byte), '')
          ...
          return (
            ...
            >
              <div className={Css.Pimg}>
                <img src={`data:image/png;base64,${First}`} alt="Product" className={Css.Proimg} />
              </div>
      ...
      <div className={Css.Pages}> <span onClick={()=>Left()}><AiFillCaretLeft className={Css.Left}/></span> {CurrentPage}   of {Pages}<span onClick={()=>Right()}><AiFillCaretRight className={Css.Left}/></span></div>
